...
async function processFetchQueue() {
    ...
    while (fetchQueue.length > 0) {
        ...
        const btn = fetchQueue.shift();

        // Double check validity (still in DOM?)
        if (document.body.contains(btn)) {
            // Skip if we accidentally queued a button that already has an answer (race condition)
            // or is handled.
            // Call fetchCorrectAnswer in silent mode
            console.log('Grasple Tools: Auto-fetching answer for button', btn);
            await fetchCorrectAnswer(btn, true);
        }
    }
        ...
        checkBtns.forEach(originalCheckBtn => {
            ...
            if (storedAnswer) {
                ...
                if (!originalCheckBtn.hasAttribute('data-grasple-auto-fetch-queued') && !parent.querySelector('.grasple-fetch-answer-btn')) {
                    ...
                    fetchQueue.push(originalCheckBtn);
    ...
    // 1. Find existing "Check" buttons
    {
        ...
        checkBtns.forEach(originalCheckBtn => {
            ...
            if (storedAnswer) {
                ...
                // If NO answer is found, inject the "Get Correct Answer" button
                // Verify not already injected
                if (!parent.querySelector('.grasple-fetch-answer-btn') && !parent.querySelector('.grasple-correct-answer-btn')) {
                    const fetchBtn = createFetchAnswerButton(originalCheckBtn.className);
                    if (!showAnswer) fetchBtn.style.display = 'none';
                    parent.insertBefore(fetchBtn, originalCheckBtn);
                    console.log('Grasple Tools: Injected Get Correct Answer button for ID:', challengeId || 'latest');
                }
            }
        });
    }
