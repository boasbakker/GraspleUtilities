...
// Dependencies: config.js, buttons.js, handlers.js

// Global Queue for Auto-Fetch
let fetchQueue = [];
let isFetchQueueRunning = false;
let currentAutoFetchDelay = 100;

async function processFetchQueue() {
    if (isFetchQueueRunning) return;
    if (fetchQueue.length === 0) return;

    isFetchQueueRunning = true;

    console.log('Grasple Tools: Starting auto-fetch queue');

    while (fetchQueue.length > 0) {
        // Wait for the delay
        await new Promise(resolve => setTimeout(resolve, currentAutoFetchDelay));

        const item = fetchQueue.shift();
        // Handle both object (new) and random legacy button pushes if any
        const btn = item.btn || item;
        const challengeId = item.challengeId || null;

        // Double check validity (still in DOM?)
        if (document.body.contains(btn)) {
            // Skip if we accidentally queued a button that already has an answer (race condition)
            // or is handled.
            // Call fetchCorrectAnswer in silent mode with explicit ID if available
            console.log('Grasple Tools: Auto-fetching answer for button', btn, 'ID:', challengeId);
            await fetchCorrectAnswer(btn, true, challengeId);
        }
    }

    isFetchQueueRunning = false;
    console.log('Grasple Tools: Auto-fetch queue finished');
}
...
async function runAnswerCheckInjection() {
    // Check settings
    const config = await configGet({ showExplanationButtons: true, showAnswerButtons: true, showHintButtons: true, autoFetchDelay: 100 });
    ...
    currentAutoFetchDelay = config.autoFetchDelay || 100;
    ...
    // 1. Find existing "Check" buttons
    {
        ...
        checkBtns.forEach(originalCheckBtn => {
            ...
            if (storedAnswer) {
                ...
                // If NO answer is found, AUTO-QUEUE automatic fetch
                if (!originalCheckBtn.hasAttribute('data-grasple-auto-fetch-queued') && !parent.querySelector('.grasple-fetch-answer-btn')) {
                    // Check deduplication
                    window.graspleFetchingIds = window.graspleFetchingIds || new Set();
                    const dedupKey = challengeId || 'latest';

                    if (window.graspleFetchingIds.has(dedupKey)) {
                        console.log('Grasple Tools: Skipping auto-fetch for already fetching ID:', dedupKey);
                        // Mark as handled to avoid repeated checks
                        originalCheckBtn.setAttribute('data-grasple-auto-fetch-queued', 'true');
                    } else {
                        // Queue the main challenge
                        window.graspleFetchingIds.add(dedupKey);
                        originalCheckBtn.setAttribute('data-grasple-auto-fetch-queued', 'true');
                        fetchQueue.push({ btn: originalCheckBtn, challengeId: challengeId });

                        // Also queue sub-challenges if present
                        // Look up the challenge to check for sub_challenges
                        const challengesMap = window.graspleChallenges || {};
                        const challengeInfo = challengeId ? challengesMap[challengeId] : null;
                        if (challengeInfo) {
                            const challengeObj = challengeInfo.data.challenge || challengeInfo.data;
                            if (challengeObj.sub_challenges && Array.isArray(challengeObj.sub_challenges)) {
                                challengeObj.sub_challenges.forEach(subChallenge => {
                                    if (subChallenge.id && !window.graspleFetchingIds.has(String(subChallenge.id))) {
                                        window.graspleFetchingIds.add(String(subChallenge.id));
                                        fetchQueue.push({ btn: originalCheckBtn, challengeId: String(subChallenge.id) });
                                        console.log('Grasple Tools: Also queued sub-challenge for fetch:', subChallenge.id);
                                    }
                                });
                            }
                        }
