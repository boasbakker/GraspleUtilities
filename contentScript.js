    ...
    */
    ...
    const observer = new MutationObserver((mutations) => {
        ...
        for (const mutation of mutations) {
            ...
            // RESTORE VISIBILITY when feedback is removed
            for (const node of mutation.removedNodes) {
                if (node.nodeType === 1) { // Element
                    // Check if it was feedback
                    let wasFeedback =
                        (node.matches && node.matches('grapp-question-feedback, [data-testid="question-feedback"], .question-feedback')) ||
                        (node.querySelector && !!node.querySelector('grapp-question-feedback, [data-testid="question-feedback"], .question-feedback'));

                    if (wasFeedback) {
                        // Find container (it's the mutation target usually)
                        const container = mutation.target.closest('grapp-question.question-wrapper') ||
                            mutation.target.closest('grapp-question') ||
                            mutation.target; // fallback

                        if (container) {
                            // Check if ANY feedback remains
                            if (!container.querySelector('grapp-question-feedback, [data-testid="question-feedback"], .question-feedback')) {
                                // Restore buttons!
                                container.querySelectorAll('.grasple-hidden-by-feedback').forEach(el => {
                                    el.classList.remove('grasple-hidden-by-feedback');
                                });
                                console.log('Grasple Tools: Restored tool visibility (feedback removed)');
                            }
                        }
                    }
                }
